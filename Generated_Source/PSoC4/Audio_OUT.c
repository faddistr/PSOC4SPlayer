...
#include "Audio_OUT.h"
    ...
    #define Audio_OUT_SetP4PinDriveMode(shift, mode)  \
    do { \
        Audio_OUT_PC =   (Audio_OUT_PC & \
                                (uint32)(~(uint32)(Audio_OUT_DRIVE_MODE_IND_MASK << \
                                (Audio_OUT_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Audio_OUT_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Audio_OUT_SetP4PinDriveMode(shift, mode)  \
        do { \
            Audio_OUT_USBIO_CTRL_REG = (Audio_OUT_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Audio_OUT_DRIVE_MODE_IND_MASK << \
                                    (Audio_OUT_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Audio_OUT_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Audio_OUT_SetP4PinDriveMode(Audio_OUT__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Audio_OUT_DR & (uint8)(~Audio_OUT_MASK));
    drVal = (drVal | ((uint8)(value << Audio_OUT_SHIFT) & Audio_OUT_MASK));
    Audio_OUT_DR = (uint32)drVal;
}
...
{
    return (uint8)((Audio_OUT_PS & Audio_OUT_MASK) >> Audio_OUT_SHIFT);
}
...
{
    return (uint8)((Audio_OUT_DR & Audio_OUT_MASK) >> Audio_OUT_SHIFT);
}
...
void Audio_OUT_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Audio_OUT_INTCFG & (uint32)(~(uint32)position);
    Audio_OUT_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Audio_OUT_INTSTAT & Audio_OUT_MASK);
	Audio_OUT_INTSTAT = maskedStatus;
    return maskedStatus >> Audio_OUT_SHIFT;
}
